...
type UnwrapSchema<Schema extends TSchema | undefined,
  Fallback = unknown> = Schema extends TSchema ? Static<NonNullable<Schema>> : Fallback
...
export interface JWTOption<Name extends string | undefined = 'jwt',
  Schema extends TSchema | undefined = undefined> extends JWSHeaderParameters,
...
) =>
{
  ...
  return new Elysia({
    ...
    verify: async (
      jwt?: string
    ): Promise<| (UnwrapSchema<Schema, Record<string, string | number>> &
      JWTPayloadSpec)
      | false> => {
      if (!jwt) return false
